...
def tricksScore(tricks):
    return 13-len(tricks)

@functools.lru_cache(maxsize=2000)
def partitionHand(hand):
    best = []
    for rank in RANKS:
        cs = [c for c in hand if c[0] == rank]
        if cs: best.append(cs)
    bestScore = tricksScore(best)
    for trick in itertools.combinations(hand, 5):
        if S5(trick) == -1: continue
        curr = [trick] + partitionHand([c for c in hand if c not in trick])
        currScore = tricksScore(curr)
