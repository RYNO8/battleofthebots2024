...
def partitionHand(hand):
    ...
    for rank in RANKS:
        ...
        if 1 <= len(cs) < 4: curr.append(cs)
        else: curr.extend((c,) for c in cs)
    partitions = [tuple(curr)]
    for trick in getAll5(hand):
        if S5(trick) == -1: continue
        for p in partitionHand([c for c in hand if c not in trick]):
            partitions.append((trick,) + p)
    return partitions
...
def play(hand, is_start_of_round, play_to_beat, round_history, player_no, hand_sizes, scores, round_no):
    ...
    * `round_no`: An integer between 0 and 9 (inclusive) indicating which round number is currently being played.
    ...
    # instawin
    otherS5 = max(list(map(S5, getAll5(other))) or [-1])
    otherS3 = max(list(map(S3, getAll3(other))) or [-1])
    otherS2 = max(list(map(S2, getAll2(other))) or [-1])
    otherS1 = max(map(S1, other))
    
    for p in partitionHand(hand):
        ...
        for trick in p:
            ...
            else: numLosing += S5(trick) <= otherS5
        if numLosing <= 1:
            print("instawin")
